import { el, eq } from '../../../../core/utils/equation.js';
...
(function () {
  ...
  function buildA(root) {
    ...
    wrap.querySelector('#z4-redp-eq').innerHTML = eq('4P + 5O₂', '2P₂O₅', '点燃');
    ...
    Object.keys(COMPS).forEach(function (k) {
      const c = COMPS[k];
      const it = el('<span class="z4-legend-item"><span class="z4-dot" style="background:' + c.color + ';color:' + c.color + '"></span>' + c.short + ' ' + c.pct + '</span>');
  ...
  function buildB(root) {
    ...
    Object.keys(SUBS).forEach(function (k) {
      const b = el('<button class="btn" data-k="' + k + '">' + SUBS[k].name + '</button>');
    ...
    [['air', '在空气中'], ['o2', '在氧气中']].forEach(function (pair) {
      const b = el('<button class="btn" data-k="' + pair[0] + '">' + pair[1] + '</button>');
    ...
    function showRecords() {
      ...
      const c1 = el('<div class="console-card accent z4-reveal"><div class="card-label">① 现象 · ' + s.name + '</div>' +
        '<div class="z4-phen">' + envTag + (isO2 ? s.phenO2 : s.phenAir) + '</div></div>');
      consoleBox.appendChild(c1);
      setTimeout(function () {
        if (tk !== recToken) return;
        consoleBox.appendChild(el('<div class="console-card accent-m z4-reveal"><div class="card-label">② 化学方程式</div>' +
          '<div class="z4-eq-wrap">' + eq(s.eq[0], s.eq[1], s.eq[2]) + '</div></div>'));
      }, 900);
      setTimeout(function () {
        if (tk !== recToken) return;
        consoleBox.appendChild(el('<div class="console-card accent-a z4-reveal"><div class="card-label">③ 注意事项</div>' +
  ...
  const _ZONE4 = { desc: desc, init: init };
  Zone4 = _ZONE4;
})();
