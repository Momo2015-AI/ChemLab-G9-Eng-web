...
export function createApplication({ state, assessment, experimentEngine, masteryService = new MasteryService(), remediationCatalog = {}, root = getDefaultRoot() }) {
  ...
  async function renderRoute(route) {
    ...
    if (route.page === 'course') {
      const dayId = route.params[0] || firstIncompleteDay(await getHomeData()) || '01';
    ...
    if (route.page === 'remediation') {
      return views.renderRemediation({ root, plan: state.learning?.remediation, onRecheck: async plan => { const ids = (plan.steps || []).find(step => step.type === 'recheck')?.knowledgeIds || []; if (await controllers.assessment.startTargeted(ids)) return router.navigate('quiz'); }, onTransfer: () => router.navigate('home') });
    }
    if (route.page === 'graph') {
      const graph = await contentService.getKnowledgeGraphViewModel();
      return views.renderGraph({ root, graph, onBack: () => router.navigate('home') });
    }
    if (route.page === 'quiz') {
      ...
      return views.renderQuiz({ root, question: session.questions[session.index], index: session.index, total: session.questions.length, onAnswer: optionIndex => { const result = controllers.assessment.answer(optionIndex); if (!result) return; renderRoute(route); } });
    }
    if (route.page === 'experiment') {
      if (!controllers.experiment.session) {
        const started = controllers.experiment.start(route.params[0]);
        if (!started) return;
      }
      const session = controllers.experiment.session;
      return views.renderExperiment({ root, experiment: session.experiment || {}, session, onNext: () => { controllers.experiment.next(); renderRoute(route); }, onObserve: text => controllers.experiment.observe(text), onComplete: () => { const result = controllers.experiment.complete(); return views.renderExperimentResult({ root, result: result || {}, onContinue: () => router.navigate('dashboard') }); } });
    }
