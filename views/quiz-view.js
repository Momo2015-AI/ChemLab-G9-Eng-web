...
export function renderQuizResult({ root, score = 0, correct = 0, total = 0, hasRemediation = false, onRemediation, onContinue, mode = 'practice', status = '', lessonId = '', answers = [], questions = [] } = {}) {
  ...
  root.querySelector('[data-transfer]')?.addEventListener('click', () => { if (lessonId && typeof window !== 'undefined') window.location.hash = `course/${lessonId}`; else router?.navigate?.('course', lessonId); });
