...
export function renderQuiz({ root, question = {}, index = 0, total = 0, onAnswer } = {}) {
  ...
  root.innerHTML = `<section class="page quiz-page"><header class="page-header"><span>Question ${index + 1} of ${total}</span><h1>${escapeHtml(question.stem || question.question || '')}</h1></header><div class="quiz-options">${options.map((option, i) => `<button type="button" data-option="${i}">${escapeHtml(option.text ?? option)}</button>`).join('')}</div></section>`;
