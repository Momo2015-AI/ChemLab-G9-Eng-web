...
export function renderQuizResult({ root, score = 0, correct = 0, total = 0, hasRemediation = false, onRemediation, onContinue, onRetry, mode = 'practice', status = '', lessonId = '', answers = [], questions = [], criteria = {}, notice = '', blocked = false } = {}) {
  if (!root) return;
  const isMastery = mode === 'mastery'; const isRecheck = mode === 'recheck'; const isTransfer = mode === 'transfer'; const passed = status === 'passed';
  const heading = blocked ? '暂不能开始答题' : isMastery ? (passed ? '本课掌握' : '还需要继续学习') : isRecheck ? (passed ? '补救完成' : '再检查一次') : isTransfer ? '迁移挑战完成' : '练习完成';
  const eyebrow = isMastery ? 'MASTERY CHECK' : isRecheck ? 'RECHECK' : isTransfer ? 'TRANSFER' : 'PRACTICE';
  const nextText = blocked ? notice : isMastery ? (passed ? '返回本课，完成最后一步' : '返回本课，查看需要补救的知识点') : isRecheck ? (passed ? '返回本课，进入95%掌握检查' : '返回本课，再次补救') : isTransfer ? '迁移完成，可以返回课程查看掌握状态' : '返回本课查看诊断与下一步学习';
  ...
  const retryAction = (typeof onRetry === 'function' && !passed && !blocked) ? `<button type="button" class="cg-btn cg-btn-ghost" data-retry>${retryLabel}</button>` : '';
  ...
  toggle?.addEventListener('click', () => { const willHide = !list.hidden; list.hidden = willHide; toggle.textContent = willHide ? `查看每题详情 (${answerItems.length} 题)` : '收起每题详情'; });
}
