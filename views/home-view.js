...
export function renderHome({ root, data = {}, onCourse, onDashboard, onGraph, onRemediation } = {}) {
  ...
  const canonicalFirstLesson = { day: '01', title: '物质的变化和性质', description: '从观察现象到证据推理，建立化学学习的第一套思维框架。' };
  const firstLessonId = canonicalFirstLesson.day;
  root.innerHTML = `
    <section class="page home-page">
      ...
      <section class="golden-lesson-entry" aria-label="精品首课">
        ...
        <div class="golden-lesson-entry__body">
          <h2>第一课：${escapeHtml(canonicalFirstLesson.title)}</h2>
          <p>${escapeHtml(canonicalFirstLesson.description)}</p>
      ...
      <section class="home-learning-path" aria-label="学习流程">
        <div class="section-heading"><span>LEARNING FLOW</span><h2>按这个顺序学习</h2><p>不要在模块之间跳来跳去。每一课都沿着同一条学习路径推进。</p></div>
      ...
      <section class="learning-center-list" aria-label="课程目录">
        <div class="section-heading"><span>CURRICULUM</span><h2>课程目录</h2><p>先学当前册次，再按课程编号顺序推进。</p></div>
        ${lessons.length ? `<div class="lesson-grid">${lessons.slice(0, 12).map((lesson, index) => `
          <button type="button" class="lesson-card" data-day="${escapeHtml(lesson.day || lesson.id)}">
            <span class="lesson-card-number">${String(index + 1).padStart(2, '0')}</span>
