...
function runtimeQuestionIds() {
  ...
  for (const entry of lessonManifest.lessons || []) {
    ...
    const mainPath = path.join(lessonsDir, `${entry.canonicalId}.json`);
    if (fs.existsSync(mainPath)) mainFiles.add(`${entry.canonicalId}.json`);
  }
  ...
  // day01 production-overrides / diagnostics also ship via ContentLoader.
...
function experimentIds() {
  ...
  // Sprint 2.5: use lessonManifest instead of the regex filter (same
  // lesson-14-law-conservation-experiment.json issue as runtime pool).
  const mainFiles = new Set();
  for (const entry of lessonManifest.lessons || []) {
    if (!entry.canonicalId) continue;
    const mainPath = path.join(lessonsDir, `${entry.canonicalId}.json`);
