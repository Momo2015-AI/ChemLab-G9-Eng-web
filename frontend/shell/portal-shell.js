...
export function mountPortalShell(root){
  ...
  const applyTheme=theme=>{
    ...
    if(toggle) toggle.textContent=theme==='light'?'☀ 浅色模式':'☾ 深色模式';
    try{ if(typeof window!=='undefined') window.localStorage.setItem(THEME_KEY,theme); }catch(e){}
  };
  let savedTheme='light';
  try{ savedTheme=(typeof window!=='undefined'&&window.localStorage.getItem(THEME_KEY))||'light'; }catch(e){}
