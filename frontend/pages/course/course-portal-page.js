export function renderCoursePortal({ root, lessons = [], onLesson = id => { window.location.hash = `course/${id}` }, onHome = () => { window.location.hash = 'home'; } } = {}) {
  if (!root) return;
  const first = lessons[0] || { day: '01', title: '物质的变化和性质', description: '从现象、性质到证据推理' };
  const firstId = first.day || first.id || '01';
  root.innerHTML = `<section class="portal-page">
    ...
    <div class="portal-grid">
      <article class="portal-card full"><h2>第一课 · Golden Lesson 01</h2><p>${escapeHtml(first.title || '物质的变化和性质')}</p><p class="portal-muted">${escapeHtml(first.description || '从观察现象到证据推理，建立化学学习的第一套思维框架。')}</p><span class="portal-chip good">95% Mastery</span><span class="portal-chip">Unseen</span><span class="portal-chip">Transfer</span><div><button class="portal-btn" data-golden>开始第一课</button></div></article>
  ...
  root.querySelector('[data-golden]')?.addEventListener('click', () => onLesson(firstId));
