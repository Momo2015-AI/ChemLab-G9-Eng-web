...
function loadLessonExperimentIds() {
  ...
  // Sprint 2.5: use the lessonManifest as the authoritative source for
  // main lesson files. The previous regex filter wrongly excluded any
  // main file whose canonical id ended in "-experiment" (e.g.
  // lesson-14-law-conservation-experiment.json).
...
function loadRuntimeQuestionIds() {
  ...
  // Sprint 2.5: same fix as loadLessonExperimentIds() — use lessonManifest
  // for main files, not the regex.
  const mainFileNames = new Set();
  for (const entry of lessonManifest.lessons || []) {
    if (!entry.canonicalId) continue;
    const mainPath = path.join(lessonsPath, `${entry.canonicalId}.json`);
  ...
  // Also pick up the day01 production-overrides / diagnostics modules.
  // These are part of the runtime question pool (ContentLoader exposes
  // them alongside lesson questions) and the graph schema audit must
  // treat their question ids as legitimate targets.
  for (const q of day01ProductionOverrides || []) if (q?.id) ids.add(q.id);
  for (const q of (day01DiagnosticQuestions || [])) if (q?.id && String(q.status || '').toLowerCase() !== 'archived') ids.add(q.id);
  return ids;
}
